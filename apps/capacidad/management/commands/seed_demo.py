...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        def sede(nombre, municipal=False):
            obj, _ = Sede.objects.get_or_create(
                codigo=nombre.lower().replace(" ", "-"),
                defaults={"nombre": nombre, "es_municipal": municipal},
            )
            return obj
        ...
        # --- Salas principales ---
        for nombre, sede_nombre, novs, metodo, terminos, overrides in SALAS_PRINCIPALES:
            sala, _ = Sala.objects.update_or_create(
                unidad_negocio=unidad, sede=sede(sede_nombre), nombre=nombre, especialidad="",
                defaults={"metodo_calculo": metodo, "atiende_sabados": True},
            )
